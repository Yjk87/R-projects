...
renderPlotly({
  df %>%
    ...
    plot_ly( x = ~batch, y = ~df[[input$numeric_variable]], color = ~Initials, type='scatter', mode='markers')%>% layout(scene = list(xaxis = list(title = 'x'),yaxis = list(title = 'y'))) %>% add_lines(y = 500, name = "linear", line = list(shape = "linear")) %>% add_lines(y = 300, name = "linear", line = list(shape = "linear"))%>% add_lines(y = 10000, name = "linear", line = list(shape = "linear")) %>% add_lines(y = 12000, name = "linear", line = list(shape = "linear"))
